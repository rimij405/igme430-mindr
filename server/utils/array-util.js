...
const hasIndex = (arr, index) => Array.isArray(arr)
    && arr.length > 0
    && index >= 0
    && index < arr.length;

// Check if array has neighboring index location.
const hasNext = (arr, index) => hasIndex(arr, index)
    && hasIndex(arr, index + 1);

// Check if array has previous index location.
const hasPrevious = (arr, index) => hasIndex(arr, index)
    && hasIndex(arr, index - 1);

// Check if index is at start of the array. False if array is empty.
const atStart = (arr, index) => Array.isArray(arr) && !isEmpty(arr) && index === 0;

// Check if index is at end of the array. False if array is empty.
const atEnd = (arr, index) => Array.isArray(arr) && !isEmpty(arr) && index === arr.length;

// Get element and perform an operation if successful.
const getElement = (arr, index) => new Promise((resolve = (element, index, array) => {},
  reject = (err) => {}) => {
  if (!arr) {

...
const createError = (status = 500, 
    ...
    message = "An unexpected error occured on the server. Please notify the system administrator.") => { 
        return new Error(status, id, message);
    };
...
const sendCustomError = (request, response, options = {
    ...
    const err = new Error(status, id, message);
